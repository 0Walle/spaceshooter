...
function Ray(owner,x,y,w,h,color,damage,dirx=0,diry=0) {
    ...
    this.outBounds = function(){
        if(this.y>canvas.height/4 || this.x<-8 || this.x>canvas.width/4 || this.y < -8){
            return true    
        }
    }
}
...
function Lifebar(total,value) {
    ...
    this.draw = function(ctx,x,y) {
        ...
        if (this.value<this.total && this.value>0)
        {
            ...
            ctx.fillStyle = '#0f0'
            ctx.fillRect(x,y,(15*this.display)/(this.total*5),1);
        }
    };
}
...
function EnBasic(x,y) {
    ...
    this.loot = function() {
        items = []
        if (Math.random()<0.1){
            for (var c_ = 0; c_ < 3; c_++) {
                if (Math.random()<0.25) items.push(new Item(coinimg,this.x+4,this.y+4,0));
                else items.push(new Item(repairimg,this.x+4,this.y+4,1));
            };
        }else{
            if (Math.random()<0.50) items.push(new Item(coinimg,this.x+4,this.y+4,0));
            else items.push(new Item(repairimg,this.x+4,this.y+4,1));
        }
...
function Item(image,x,y,type) {
    ...
    this.update = function() {
        ...
        if(this.x > canvas.width/4-8) this.velx *= -1
